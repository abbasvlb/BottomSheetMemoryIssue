...
function CloseCall({ navigation }: { navigation: any }) {
  ...
  const { productList, clearPrducts } = useContext(AppContext);
...
function OrderHome({ navigation }: { navigation: any }) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{'Order Screen'}</Text>
      <Text>{`${products.length} data loaded.`}</Text>
...
function RetailerActivity({ navigation }: { navigation: any }) {
  //const dispatch = useDispatch();
  const { setProducts, clearPrducts } = useContext(AppContext);
...
const AppProvider = ({ children }: { children: any }) => {
  ...
  function clearPrducts() {
    setProductList([]);
  }

  // Values to be provided to consumers
  const contextValue = {
    productList, setProducts, clearPrducts,
  };
